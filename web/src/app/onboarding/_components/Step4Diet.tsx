...
export const Step4Diet = () => {
  ...
  const handleNext = async () => {
    const isValid = await trigger(['diet']);
    if (isValid) {
      setStep(5);
    }
  };
  ...
  return (
    <div className='flex flex-col h-full max-w-6xl mx-auto px-4 pb-6 lg:pb-10 w-full'>
      <div className='flex-shrink-0 space-y-2 mb-6'>
        <h3 className='text-3xl font-bold'>Chế độ ăn</h3>
        <p className='text-muted-foreground'>
          Chọn chế độ ăn phù hợp với nhu cầu và sở thích của bạn.
        </p>
      </div>
